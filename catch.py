...
def path_to_commands(path, initial_direction='UP'):
    ...
    def flush_forward_local(count):
        if count == 1:
            cmds.append("앞으로")
        elif count > 1:
            cmds.append(f"앞으로 {count}칸")
    ...
    for i in range(1, len(path)):
        ...
        if direction == target_dir:
            ...
            flush_forward_local(forward_count)
            forward_count = 0
...
def main():
    ...
    if 'state' not in st.session_state:
        ...
        start, obstacles, goals, portals = generate_map(
            level_info['obstacles'],
            use_portals=level_info.get('portals', False)
        )
...
try:
    ...
    st.error("앱 초기화 중 예외가 발생했습니다. 아래 로그를 확인하세요.")
    st.code(traceback.format_exc())
