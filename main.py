...
with st.expander("📘 게임 설명 보기"): st.markdown("""
### 🎮 게임 방법 로봇 🤡에게 명령어를 입력하여 두 개의 🎯 목표 지점에 도달하고 집기 명령으로 수집하세요!
         장애물(⬛)을 피하고, 귀신(👻)에게 잡히지 않도록 조심하세요! 
### ✏️ 사용 가능한 명령어 (기본 방향 위)
        - 편의를 위한 자동완성 명령어 기능 존재    
        - 앞으로 : 위로 1칸 이동
        - 앞으로 2칸, 앞으로 3칸 : 위로 여러 칸 이동
        - 왼쪽으로 이동 : 왼쪽 방향으로 1칸 이동 
        - 오른쪽으로 이동 : 오른쪽 방향으로 1칸 이동
        - 뒤로 이동 : 아래로 1칸 이동
        - 집기 : 현재 칸에 목표물이 있을 경우 수집
        
### 🌀 포탈 - 포탈(🌀)에 들어가면 다른 포탈 근처 랜덤 위치로 순간 이동 
            - 귀신은 포탈을 사용할 수 없음 

### 👻 귀신 - 레벨 4: 귀신은 장애물을 피해서 이동 
            - 레벨 5: 귀신은 장애물을 무시하고 직진 추적 
            
### 🏆 Perfect 판정 - 최단 경로 + 모든 목표 수집 + 명령 수 최소일 때 Perfect! 🌟 

### 🧱 각 레벨 정보 - Level 1 (5점, 착한맛): 장애물 8개, 귀신 없음 
